 ...
 */
export const PARAMETER_REGEX =
  /<parameter\s+name="([^"]+)">([\s\S]*?)<\/antml:parameter>/g;

/**
 * Matches tool results in the response.
 * Captures: [1] tool call ID, [2] result content
 */
export const TOOL_RESULT_REGEX =
  /<tool_result\s+tool_call_id="([^"]+)"(?:\s+is_error="(true|false)")?>([\s\S]*?)<\/antml:tool_result>/g;

/**
 * Matches the entire function_calls block for stripping.
 * Used to remove tool call XML from display content.
 */
export const STRIP_TOOL_CALL_REGEX =
 ...
 */
export const STRIP_TOOL_RESULTS_REGEX =
  /<tool_result[\s\S]*?<\/antml:tool_result>/g;
 ...
 * ```
 */
export function parseToolResults(text: string): ParsedToolResult[] {
 ...
 */
export function hasToolResults(text: string): boolean {
  TOOL_RESULT_REGEX.lastIndex = 0;
