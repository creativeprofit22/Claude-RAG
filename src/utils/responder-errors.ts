 ...
 */
export type ResponderErrorCode =
  ...
  | 'NOT_FOUND'     // CLI not installed or not in PATH
...
export class ResponderError extends Error {
  constructor(
    message: string,
    public readonly code: ResponderErrorCode,
    public readonly responder: 'claude' | 'gemini'
  ) {
    super(message);
...
export function classifyCliError(
  ...
  if (code === null) {
    return new ResponderError(
      'Claude Code CLI was terminated by a signal',
      'UNKNOWN',
      'claude'
    );
  }
 ...
 */
export function classifyGeminiError(error: unknown, timeoutMs?: number): ResponderError {
  ...
  if (error instanceof Error) {
    ...
    if (msg.includes('api key') || msg.includes('apikey') || msg.includes('authentication')) {
      return new ResponderError(
        'Invalid or missing GOOGLE_AI_API_KEY',
        'AUTH',
        'gemini'
      );
    }
    ...
    if (msg.includes('quota') || msg.includes('rate limit') || msg.includes('429')) {
      return new ResponderError(
        'Gemini API quota exceeded. Try again later or check your usage limits.',
        'RATE_LIMIT',
        'gemini'
      );
    }
    ...
    if (msg.includes('safety') || msg.includes('blocked')) {
      return new ResponderError(
        'Response blocked by Gemini safety filters. Try rephrasing your query.',
        'SAFETY',
        'gemini'
      );
    }
    ...
    if (msg.includes('timeout') || msg.includes('timed out')) {
      ...
      return new ResponderError(
        `Gemini API request timed out${timeoutInfo}`,
        'TIMEOUT',
        'gemini'
      );
    }
 ...
 */
export function cliNotFoundError(): ResponderError {
  return new ResponderError(
    'Claude Code CLI is not installed or not in PATH. Install it with: npm install -g @anthropic-ai/claude-code',
    'NOT_FOUND',
    'claude'
  );
}
 ...
 */
export function createTimeoutError(operation: string, ms: number): ResponderError {
  return new ResponderError(
