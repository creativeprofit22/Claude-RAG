...
import type {
  ...
  TypewriterState,
} from './typewriter.types.js';
import { useTypewriterSound } from './useTypewriterSound.js';
import { TypewriterSVG, type TypewriterSVGRefs } from './TypewriterSVG.js';
...
export function TypewriterInput({
  ...
  const svgRef = useRef<TypewriterSVGRefs>(null);
   ...
   */
  const calculateCarriagePosition = useCallback((text: string): number => {
   ...
   */
  const animateCarriageShift = useCallback(
    (direction: 'left' | 'right') => {
      ...
      const newPosition = Math.max(
        0,
        state.carriagePosition + (direction === 'left' ? CHAR_WIDTH : -CHAR_WIDTH)
      );
   ...
   */
  const animateCorrectionTape = useCallback(
    (rapid: boolean = false) => {
      const correctionArm = svgRef.current?.correctionArm;
      if (!correctionArm) return;
  ...
  return (
    <div
      ref={containerRef}
      className={`typewriter-svg-container ${disabled ? 'disabled' : ''} ${className}`}
    >
      {/* SVG Visual - The detailed typewriter illustration */}
      <TypewriterSVG
        ref={svgRef}
        showKeyboard={showKeyboard}
        className="typewriter-svg-visual"
      />

      {/* Textarea overlay - positioned over the paper area of the SVG */}
      <textarea
        ref={textareaRef}
        className="typewriter-svg-textarea"
        value={value}
