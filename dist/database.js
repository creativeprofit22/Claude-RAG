 ...
 */
function toVectorDocuments(results) {
    return results;
}
/**
 ...
 */
function toSearchResults(results) {
    return results;
}
/**
 ...
 */
async function withTableRaceRetry(operation, retryOperation) {
    try {
...
class RAGDatabase {
     ...
     */
    async withTable(fallback, callback) {
     ...
     */
    async paginateTable(table, processRow, batchSize = 5000) {
        let offset = 0;
        let hasMore = true;
        while (hasMore) {
            try {
    ...
    async addDocuments(docs) {
        ...
        const db = this.db;
        this.table = await withTableRaceRetry(async () => {
     ...
     */
    async getDocumentChunks(documentId, limit = DEFAULT_CHUNK_LIMIT) {
    ...
    async getDocumentSummaries() {
        return this.withTable([], async (table) => {
            ...
            await this.paginateTable(table, (doc) => {
                ...
                if (!doc || !doc.metadata || !doc.metadata.documentId)
                    return;
     ...
     */
    async getDocumentDetails(documentId, limit = DEFAULT_CHUNK_LIMIT) {
